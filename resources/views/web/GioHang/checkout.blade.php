                                    ...
                                    @foreach($products as $sp)
                                        <tr>
                                            <td class="image product-thumbnail"><img
                                                    src="{{ asset($sp->san_pham->hinhAnh->hinh_anh) }}" alt="#"></td>
                                            <td>
                                                <h5><a href="">{{ $sp->san_pham->ten_san_pham }}</a>
                                                    <span class="product-qty" data-product-id="{{ $sp->san_pham->id }}">x{{ $sp->so_luong }}</span>
                                                </h5>
                                            </td>
                                            <td>{{ number_format($sp->san_pham->gia_ban, 0, ',', '.') }} VND</td>
                                        </tr>
                                        @php
                                            $thanhTien += $sp->so_luong * $sp->san_pham->gia_ban;
                                        @endphp
                                    @endforeach
                                    <tr>
                                        <th>Phí giao hàng</th>
